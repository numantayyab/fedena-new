...
class RspecControllerGenerator < ControllerGenerator
  ...
  def manifest
    record do |m|
      ...
      m.directory File.join('app_p/controllers', class_path)
      m.directory File.join('app_p/helpers', class_path)
      m.directory File.join('app_p/views', class_path, file_name)
      ...
      m.template 'controller:controller.rb',
        File.join('app_p/controllers', class_path, "#{file_name}_controller.rb")

      m.template 'controller:helper.rb',
        File.join('app_p/helpers', class_path, "#{file_name}_helper.rb")
      ...
      actions.each do |action|
        ...
        path = File.join('app_p/views', class_path, file_name, "#{action}.#{@default_file_extension}")
