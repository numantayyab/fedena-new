...
class RspecScaffoldGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      m.directory(File.join('app_p/models', class_path))
      m.directory(File.join('app_p/controllers', controller_class_path))
      m.directory(File.join('app_p/helpers', controller_class_path))
      m.directory(File.join('app_p/views', controller_class_path, controller_file_name))
      m.directory(File.join('app_p/views/layouts', controller_class_path))
      ...
      m.template("scaffold:layout.html.erb", File.join('app_p/views/layouts', controller_class_path, "#{controller_file_name}.html.erb"))
      ...
      m.template "scaffold:controller.rb",
        File.join('app_p/controllers', controller_class_path, "#{controller_file_name}_controller.rb")
      ...
      m.template "scaffold:helper.rb",
        File.join('app_p/helpers', controller_class_path, "#{controller_file_name}_helper.rb")

      for action in scaffold_views
        m.template(
          "scaffold:view_#{action}.#{@default_file_extension}",
          File.join('app_p/views', controller_class_path, controller_file_name, "#{action}.#{default_file_extension}")
        )
      end
      
      # Model class, unit test, and fixtures.
      m.template 'model:model.rb',            File.join('app_p/models', class_path, "#{file_name}.rb")
